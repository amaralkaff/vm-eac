...
// External g_FeatureManager from main.cpp
...
namespace RenderCore
{
    ...
    void OSImGui_External::MainLoop()
    {
        while (!EndFlag)
        {
            ...
            g_Device.g_pSwapChain->Present(1, 0); // Present with vsync enabled to reduce CPU usage
        }
        CleanImGui();
    }
    ...
    bool OSImGui_External::CreateMyWindow()
    {
        ...
        // Set window handle for RegisterHotKey
        if (g_FeatureManager && Window.hWnd != NULL) {
            g_FeatureManager->SetTargetWindow(Window.hWnd);
        }
    ...
    LRESULT WINAPI WndProc_External(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
    {
        ...
        {
        case WM_HOTKEY:
            // Handle RegisterHotKey messages
            if (g_FeatureManager) {
